...
ysy.view.extender(ysy.view.Widget, ysy.view.GanttTask, {
  _repaintCore: function () {
    ...
    if (gantt._pull[gantt_issue.id]) {
      ...
      if (gantt_issue.parent !== 0 && !gantt._pull[gantt_issue.parent]) {
        this.dhdata.realParent = gantt_issue.parent;
      }
  ...
  _constructDhData: function (issue) {
    ...
    ysy.proManager.extendGanttTask(issue, gantt_issue);
    return gantt_issue;
  },
  update: function (item, keys) {
    ...
    if (item.type === "project") {
      obj = {
        ...
        _shift: Math.round(item.shift / (60 * 60 * 24))
      };
      this.model.set(obj);
    } else {
      var fullObj = {
