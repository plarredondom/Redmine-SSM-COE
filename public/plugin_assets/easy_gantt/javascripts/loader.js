...
$.extend(ysy.data.loader, {
  ...
  init: function () {
    ...
    settings.sample = new data.Sample();
  ...
  _loadRelations: function (json) {
    ...
    for (var i = 0; i < json.length; i++) {
      ...
      if (!allowedTypes[json[i].type]) continue;
  ...
  _loadHolidays: function (json) {
    if (!json) return;
    var limits = ysy.data.limits;
    limits.holidays = json;
    limits._fireChanges(this, "load holidays");
  },
