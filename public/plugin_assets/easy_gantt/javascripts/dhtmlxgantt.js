...
gantt._tasks_dnd = {
  ...
  _resize : function(ev, shift, drag){
    ...
    if(drag.left){
      ...
      if (!gantt.ascStop(ev, start_date)) {
        return;
      }
      ...
      if (!gantt.ascStop(ev, undefined, end_date)) {
        return;
      }
    ...
    if(drag.left){
      gantt.moveDesc(ev, ev.start_date.diff(old_start, "seconds"), true, 'left');
    }else{
      gantt.moveDesc(ev, ev.end_date.diff(old_end, "seconds"), true, 'right');
    }
  ...
  _move : function(ev, shift, drag){
    ...
    if (ev.type === "project") {
      ...
      gantt.moveDesc(ev, diff, true);
      gantt.moveChildren(ev, diff);
    }
